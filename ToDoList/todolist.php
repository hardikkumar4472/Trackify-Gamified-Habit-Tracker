...
$db = new mysqli('localhost', 'root', '', 'trackify');
if ($db->connect_error) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db->prepare("INSERT INTO tasks (user_id, name, priority, category_id, due_date, completed, created_at) VALUES (?, ?, ?, ?, ?, 0, NOW())");
    $stmt->bind_param("issis", $user_id, $name, $priority, $category_id, $due_date);
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    if ($taskId) {
        $stmt = $db->prepare("UPDATE tasks SET completed = ? WHERE id = ? AND user_id = ?");
        $stmt->bind_param("iii", $completed, $taskId, $user_id);
        $stmt->execute();
        $stmt->close();
